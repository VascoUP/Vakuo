...
public class EnemyController : MonoBehaviour
{
    ...
    private void OnEnable() 
    {
        ...
        ChangeState(EnemyStates.PLAYER_ENTERED);
    }
        ...
        if(!runningState) {
            switch(_state) {
                ...
                case EnemyStates.MOVE_TO_PLAYER:
                    runningCoroutine = WalkToTarget(target, walkSpeed, distanceToCharge);
                    break;
                case EnemyStates.WAIT_CHARGE:
                    runningCoroutine = PrepareChargeAttack(prepareChargeDuration);
                    break;
                case EnemyStates.CHARGE:
                    runningCoroutine = ChargeAttack(target.position - transform.position, chargeSpeed, chargeDuration);
                    break;
